...
class EstateLease(models.Model):
    ...
    name = fields.Char("Lease Ref", readonly=True, copy=False, default='New', index=True)
    property_id = fields.Many2one('estate.property', required=True, ondelete='restrict', tracking=True, index=True)
    ...
    subletting_approved = fields.Boolean("Subletting Approved", default=False)
    ...
    wht_amount = fields.Monetary("WHT Amount (KES/mo)", currency_field='currency_id', compute='_compute_wht', store=True)
    net_rent = fields.Monetary("Net Rent after WHT", currency_field='currency_id', compute='_compute_wht', store=True)
    landlord_payout = fields.Monetary("Landlord Payout (KES/mo)", currency_field='currency_id', compute='_compute_wht', store=True)
    ...
    @api.depends('payment_ids.payment_state', 'payment_ids.amount_total',
    ...
    def _check_dates(self):
        for r in self:
            if r.date_end <= r.date_start:
                raise ValidationError(_("Lease end date must be after start date."))
    ...
    def _check_no_overlap(self):
        for r in self:
            ...
            if r.unit_id:
                domain.append(('unit_id', '=', r.unit_id.id))
            else:
                domain.append(('property_id', '=', r.property_id.id))
            overlap = self.search(domain, limit=1)
            if overlap:
                raise ValidationError(
                    _(f"Overlapping lease: {overlap.name} ({overlap.date_start} to {overlap.date_end})")
                )

    def action_activate(self):
        for r in self:
            if r.deposit and not r.deposit_paid:
                raise UserError(_("Security deposit must be paid before activation."))
            ...
            r.message_post(body=_(
                ...
                f"Rent: KES {r.monthly_rent:,.0f}. Term: {r.date_start} to {r.date_end}."
            ))
    ...
    @api.model
    def _cron_auto_monthly_invoices(self):
        today = fields.Date.today()
        if today.day != 1:
            return
    ...
    def _cron_expiry_reminders(self):
        ...
        for days in (60, 30, 7):
            ...
            for l in leases:
                l.message_post(
                    body=_(f"Lease expires in {days} days ({l.date_end}). "
                           f"Tenant: {l.tenant_id.name}. Rent: KES {l.monthly_rent:,.0f}."),
