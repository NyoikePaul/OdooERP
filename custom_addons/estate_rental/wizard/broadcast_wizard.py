...
class EstateBroadcastWizard(models.TransientModel):
    _name        = 'estate.broadcast.wizard'
    _description = 'Broadcast Message to Tenants'

    scope       = fields.Selection([
        ('all','All Active Tenants'),('building','Building Tenants'),
        ('county','County Tenants'),('arrears','Tenants in Arrears'),
    ...
    def _compute_count(self):
        for r in self:
            r.tenant_count = len(r._get_leases())
    ...
    def _get_leases(self):
        ...
        if self.scope=='building' and self.building_id:
            ...
            domain.append(('property_id.county','=',self.county))
        elif self.scope=='arrears':
            domain.append(('months_arrears','>',0))
        return self.env['estate.lease'].search(domain)

    def action_broadcast(self):
        leases = self._get_leases()
        if not leases:
            raise UserError(_("No tenants found."))
        for l in leases:
            l.message_post(subject=self.subject, body=self.body,
                           partner_ids=[l.tenant_id.id], subtype_xmlid='mail.mt_comment')
        return {'type':'ir.actions.client','tag':'display_notification',
                'params':{'title':_('Broadcast Sent'),
