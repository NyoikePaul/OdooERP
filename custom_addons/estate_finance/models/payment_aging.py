"""
Tenant Payment Aging Analysis — Kenya
...
class EstatePaymentAging(models.Model):
    ...
    current        = fields.Float("Current (not due)",    readonly=True)
    ...
    total_outstanding = fields.Float("Total Outstanding", readonly=True)
    ...
    def init(self):
        self.env.cr.execute("""
            ...
            SELECT
                ...
                el.property_id    AS property_id,
                am.lease_id       AS lease_id,
            ...
            LEFT JOIN estate_lease el ON el.id = am.lease_id
            WHERE am.move_type = 'out_invoice'
              AND am.state = 'posted'
              AND am.payment_state NOT IN ('paid','reversed')
              AND am.amount_residual > 0
            GROUP BY am.partner_id, el.property_id, am.lease_id, am.currency_id
